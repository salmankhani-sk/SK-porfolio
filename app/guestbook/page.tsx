...
export default function GuestbookPage(){
    return(
        <section className="max-w-7xl w-full px-4 md:px-8 mx-auto">
            <h1 className="text-4xl font-semibold lg:text-5xl pt-5" >Guest Book</h1>
    ...
    return data.map((item) => (
      <li key={item.id}>
        <div className="flex items-center">
          ...
          <p className="text-muted-foreground pl-3 break-words">{item.User?.firstname}:<span> {item.message}</span></p>
        </div>
      </li>
    ));
  }
